...
This workflow automates the process of creating a GitHub release for an application (APK) and updating the local `apps.json` metadata for the MasconnStore project (app-market-release).
...
3. **Extract App Icon**
   - Run `aapt dump badging "dist/[FILENAME]" | grep "icon=" | cut -d"'" -f2` to find the icon path (stored as `ICON_PATH`).
   - Run `unzip -p "dist/[FILENAME]" "[ICON_PATH]" > "dist/icon.png"`.
...
5. **Upload Assets**
   - Run `gh release upload [VERSION_TAG] "dist/[FILENAME]" -R 61160306/storeupdate` for the APK.
   - Run `gh release upload [VERSION_TAG] "dist/icon.png" -R 61160306/storeupdate` for the icon.
...
9. **Cleanup**
   Run `rm dist/[FILENAME] dist/icon.png` to remove local files.
